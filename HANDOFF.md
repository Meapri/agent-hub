...
- **현재 단계**: 3.1.0을 병합·설치해 daemon과 bridge 모두 `3.1.0-a8f0f45fb09e`, doctor 7/7 pass입니다. 그 뒤 이 인계 기록을 갱신하다가 handoff 도구에서 같은 종류의 결함을 발견해 3.1.1로 고쳤습니다. 3.1.1은 아직 설치 전입니다.
- **완료**:
...
- **미완**: provider MCP 프로토콜 계층 2,228줄은 그대로입니다. v2는 이제 전혀 import하지 않지만 `scripts/*_mcp.py` launcher가 실제로 동작하므로 삭제는 사용자 판단이 필요합니다. 3.1.1 릴리스 스테이징과 설치가 남았습니다.
...
- **검증 실행 결과**: 전체 pytest `853 passed, 2 skipped`; `ruff check` 통과; 작업한 4개 파일 format 통과; `./scripts/check-sync.sh` 통과; README verify·document_quality 통과; 공개 도구 14개 유지. 고침을 되돌려 새 테스트가 실패하는 것을 확인했습니다(`file` 인자 1건, 스키마 문서 4건). 3.1.0 기준 live canary는 19 passed였고 3.1.1로는 아직 돌리지 않았습니다.
...
- **다음 한 걸음**: `./.venv/bin/agent-hub stage-release --repo-root . --json`으로 3.1.1을 검토한 뒤 `--apply --proposal-sha256`, `agent-hub update --candidate-root`, `agent-hub setup --runtime-root`까지 세 단계를 모두 실행해 설치하세요.
