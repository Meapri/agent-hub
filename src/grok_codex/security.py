...
def consent_file_path() -> Path:
    override = os.getenv("GROK_CODEX_CONSENT_FILE", "").strip()
    if override:
...
def user_consent_enabled() -> bool:
    if env_flag("GROK_CODEX_USER_CONSENT"):
        return True
    try:
...
def consent_status() -> dict:
    ...
    return {
        ...
        "configuration": {
            ...
            "revoke_command": "python3 scripts/grok_codex_consent.py revoke",
            "enable_all": "GROK_CODEX_USER_CONSENT=1",
        },
    }
