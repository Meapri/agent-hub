...
2. Check the plugin version, MCP `initialize`, and `tools/list`.
...
7. Do not treat a saved login or model catalog alone as proof that generation
   works; after login, use the explicit generation check when requested.
...
Never inspect browser state or Keychain. Never print tokens, secrets,
