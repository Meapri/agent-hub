...
1. 먼저 `agent_hub_status`와 `provider="all"`로 현재 동의·계정 로그인·token 준비 상태를 읽는다.
   `logged_in`은 계정 연결, `auth_ready`는 현재 호출 가능한 인증, `ready`는 동의와 provider 준비까지
   끝난 상태다. `refreshable`을 로그인 해제로 해석하지 않는다.
...
5. `refreshable=true`이면 사용자가 GUI의 **세션 갱신**을 직접 누르게 한다. 상단 **상태 다시 확인**과
   routine `agent_hub_status`는 credential을 갱신하지 않는다. `relogin_required=true`일 때만 공식
   로그인 흐름을 다시 안내한다.
6. 로그인이나 갱신 뒤에는 GUI의 연결 테스트나 `agent_hub_status`로 준비 상태를 다시 확인한다. Gemini의
...
7. 모델을 바꿀 때는 provider의 로컬 안전 목록을 먼저 보여 주고, 연결이 준비된 경우에만 최신 목록을
...
- 세션 갱신 결과나 provider exception 원문을 job에 넣지 않는다. GUI에는 고정된 성공·실패 문구만
  표시하고 credential revision, token prefix, 계정 식별자를 노출하지 않는다.
