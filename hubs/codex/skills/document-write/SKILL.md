...
2. 저장소 전체를 설명하는 문서라면 먼저 `agent_hub_plan_workflow`의 `adaptive` workflow로 계획만
   ...
   장문 조사·작성 wave는 `background=true`로 접수하고 `agent_hub_get_run`으로
   `lease_active=false`가 될 때까지 상태를 확인한다. 실행 중인 lease에 continue를 다시 호출하지 않고,
   완료 후 반환된 revision으로 다음 wave를 잇는다.
...
4. 생성 결과의 `quality_gate.applied`와 `quality_gate.passed`를 확인한다. `passed=false`이거나 도구 응답이
   ...
   경고를 반영해 전체 문서를 다시 만든다. adaptive plan에서 생성된 문서 초안을 검토할 때는
   dependency 텍스트를 받는 `review_text`를 사용한다. `review_diff`는 Git working tree 전용이므로
   아직 파일로 저장하지 않은 초안 검토에 사용하지 않는다.
