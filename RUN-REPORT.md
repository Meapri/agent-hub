...
기능을 빠짐없이 설명하는 데 치우쳤던 README를 실제 사용 순서대로 다시 썼다. 첫 화면에서 내부 구조와
추상적인 장점을 나열하지 않고, 여러 모델을 오가며 생기는 불편과 이 도구를 만든 이유를 짧게 설명한다.
설치, 모델 로그인, 앱 연결을 앞에 두고 공개 도구와 내부 실행 규칙은 뒤로 옮겼다. 대칭적인 불릿과
번역투를 줄이고, 명령 앞뒤에는 왜 실행하는지 필요한 만큼만 덧붙였다.

Agent Hub adaptive plan도 함께 사용했다. plan SHA는
`fc9dcf1a310679c7c8f2d0ba6f71df82fd58d654fa9737cf4fdea9f82bcc6e9e`, run ID는
`9b67815a45f9`다. Claude Sonnet 5의 deep/high 조사는 후보 255개 중 58개 파일을 읽고 약 143초 만에
끝났다. 반면 기존 README 문체를 보도록 계획된 Gemini 단계는 `review_diff`를 골라, 깨끗한 작업 트리에서
`empty_diff`만 반환했다. 문서 본문 검토 결과로는 사용할 수 없는 응답이다. 마지막 작성 단계도 약 282초
뒤 전체 시간 예산을 소진해 pause됐으며 완성된 텍스트가 없었다. 이 두 결과는 README에 반영하지 않았다.

최종 문서는 코드 조사에서 확인한 사실과 기존의 필수 명령을 바탕으로 직접 작성했다. 자동 품질 검사와
현재 소스의 `agent_hub_verify` checker v3는 모두 경고 0건으로 통과했다. README 계약·provider 문서 품질
테스트는 `23 passed`, `git diff --check`도 통과했다. 앱 브라우저의 로컬 `file://` 접근 제한은 그대로여서
렌더링 화면 검수는 수행하지 못했다.
